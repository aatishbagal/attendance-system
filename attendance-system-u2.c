...
void mark_attendance(int classIndex) {
    ...
    while (1) {
        // Display the menu
        for (int i = 0; i < classes[classIndex].studentCount; i++) {
            mvprintw(3 + i, 1, "[%c] %s", newRecord.attendance[i], classes[classIndex].students[i].name);
            if (i == selected) {
                mvprintw(3 + i, 0, "*"); // Highlight the selected item
            } else {
                mvprintw(3 + i, 0, " "); // Remove highlight for others
            }
        }
        
        mvprintw(3 + classes[classIndex].studentCount, 1, "Save and Exit");
        mvprintw(4 + classes[classIndex].studentCount, 1, "Discard and Exit");
